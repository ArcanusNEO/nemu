...
static void send_packet(FILE* out, const uint8_t* command, size_t size) {
  ...
  if (ferror(out)) err(1, "send");
  else if (feof(out)) errx(0, "send: Connection closed");
}
...
static uint8_t* recv_packet(FILE* in, size_t* ret_size, bool* ret_sum_ok) {
  ...
  if (ferror(in)) err(1, "recv");
  else if (feof(in)) errx(0, "recv: Connection closed");
  else errx(1, "recv: Unknown connection error");
}
