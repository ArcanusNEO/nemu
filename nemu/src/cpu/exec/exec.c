...
#define make_group(                                                       \
  name, item0, item1, item2, item3, item4, item5, item6, item7)           \
  static opcode_entry concat(opcode_table_, name)[8] = {/* 0x00 */ item0, \
    item1, item2, item3, /* 0x04 */ item4, item5, item6, item7};          \
...
/* 0x80, 0x81, 0x83 */
make_group(
  gp1, EX(add), EX(or), EX(adc), EX(sbb), EX(and), EX(sub), EX(xor), EX(cmp));

/* 0xc0, 0xc1, 0xd0, 0xd1, 0xd2, 0xd3 */
make_group(
  gp2, EX(rol), EX(ror), EMPTY, EMPTY, EX(shl), EX(shr), EMPTY, EX(sar));

/* 0xf6, 0xf7 */
make_group(gp3, IDEXW(test_I, test, 1), EMPTY, EX(not ), EX(neg), EX(mul),
...
/* 0xff */
make_group(gp5, EX(inc), EX(dec), IDEX(J_gp5, call), EMPTY, IDEX(J_gp5, jmp),
  EMPTY, EX(push), EMPTY);
...
opcode_entry opcode_table[512] = {
  ...
  /* 0x88 */ IDEXW(mov_G2E, mov, 1), IDEX(mov_G2E, mov), IDEXW(mov_E2G, mov, 1),
  IDEX(mov_E2G, mov),
  ...
  /* 0xb4 */ EMPTY, EMPTY, IDEXW(E2G, movzx, 2), IDEXW(E2G, movzx, 2),
  /* 0xb8 */ EMPTY, EMPTY, EMPTY, EMPTY,
  /* 0xbc */ EMPTY, EMPTY, IDEXW(E2G, movsx, 2), IDEXW(E2G, movsx, 2),
